...
VIDEO_DIR = 'FilmAssets' 
# Tider for agent-animation (baseret på 18 sek. video)
T_START_IN = 0.0
T_END_IN = 1.5
T_START_OUT = 9.5 
T_END_OUT = 11.0
...
# Tidspunkt for væggen fjernes/responsen skal starte 
RESPONSE_START_TIME = 16.0 
RESPONSE_DURATION = 2.0 

# KONSTANT
NUM_BUFFER_TRIALS = 2 # Antal buffer forsøg i starten af hver blok

# KONSTANTER FOR ATTENTION CHECK 
...
class EM2Experiment:
    def __init__(self):
        ...
        self.num_trials_per_block = 40 # 8 main trials
        self.num_blocks = 2 
    ...
    def setup_visual_stimuli(self):
        ...
        if self.win is None:
            self.win = visual.Window(size=[1920, 1080], color='black', fullscr=True, monitor='testMonitor', units='pix') 
    ...
    def _block_transition(self, block_num):
        ...
        agent_type = f"Egen-Agent ({self.subject_name})" 
